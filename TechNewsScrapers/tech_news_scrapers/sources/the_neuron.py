from __future__ import annotations

from ..models import NewsItem
from ..utils import feed_entries_to_items, first_working_feed_url


def scrape_the_neuron(limit: int = 50) -> list[NewsItem]:
    candidates = [
        "https://www.theneurondaily.com/feed",
        "https://www.theneurondaily.com/rss",
        "https://www.theneurondaily.com/rss.xml",
        "https://theneurondaily.com/feed",
    ]
    feed_url = first_working_feed_url(candidates)
    return feed_entries_to_items(
        feed_url=feed_url,
        source_id="the_neuron",
        source_name="The Neuron",
        limit=limit,
    )

